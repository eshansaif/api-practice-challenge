...
document.getElementById('car-quantity').innerText = data.length;

const showCars = () => {
  const carContainer = document.getElementById('car-container');
  data.forEach(car => {
    console.log(car);
    const { imageURL, name, description, price } = car;
    const div = document.createElement('div');
    div.classList.add('col');
    div.innerHTML = `
    <div class="card">
    <img src="${imageURL ? imageURL : 'https://upload.wikimedia.org/wikipedia/commons/1/14/No_Image_Available.jpg'}" class="card-img-top" alt="..." width="590px !important" height="305px !important">
    <div class="card-body">
        <h5 class="card-title">Car Name: ${name}</h5>
        <p class="card-text">Description: ${description}</p>
    </div>
    <div class="card-footer">
        <a href="#" class="btn btn-primary">Buy this car at Price: ${price}</a>
    </div>
</div>
    `;
